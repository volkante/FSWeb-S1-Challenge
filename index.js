...
//console.table(fenomenSil(fenomenler,1));
...
function fenomenEkle(arr, number, profile, followers, posts, platform ) {
    ...
    copyArr.push(fenomen);
    return copyArr;
}

//console.table(fenomenEkle(fenomenler, 6, "Workintech", 10000000, 2022, "Instagram"))
...
function enFenomenler(dizi) {
  const hundredMillionFollower = []
  for (let i = 0; i < dizi.length; i++) {
    if (dizi[i].followers > 100000000){
      hundredMillionFollower.push(dizi[i].profile)
    };
  }
  return hundredMillionFollower;
}
// console.table(enFenomenler(fenomenler))
...
function fenomenGonderimSayisi(arr, profileInput){
    for (let i = 0; i < arr.length; i++) {
      if (arr[i].profile === profileInput){
        return arr[i].posts
      }
    }
}

// console.log(fenomenGonderimSayisi(fenomenler, 'Barack Obama'))
...
3. Verilen platformda en çok gönderi yapan fenomen adını döndürün
...
function platformaGoreCokGonderiYapanFenomen(arr, platform) {
  ...
  let maxGonderen = "";
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].platform === platform && arr[i].posts > maxGonderi) {
    maxGonderi = arr[i].posts;
    maxGonderen = arr[i].profile;    
    }
  }
  return maxGonderen
}
